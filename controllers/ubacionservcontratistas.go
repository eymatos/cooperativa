...
func GetUbicacionServContratistas(w http.ResponseWriter, r *http.Request) {

	db, err := sqlx.Connect("mysql", configuraciones.ConnectionString)
	query := `select id, id_servicessupplier, id_country, id_state, id_city, available from ServicesSupplierLocations;`

	q_UbicacionServContratistas := []models.UbicacionServContratistas{}
	db.Select(&q_UbicacionServContratistas, query)
	...
	json.NewEncoder(w).Encode(q_UbicacionServContratistas)
	db.Close()
}

...
func GetListarPaisesCliente(w http.ResponseWriter, r *http.Request) {

	db, err := sqlx.Connect("mysql", configuraciones.ConnectionString)
	query := `select countries.id, countries.name, countries.iso3, countries.numeric_code, countries.iso2, countries.phonecode, 
	countries.capital, countries.currency, countries.currency_name, countries.currency_symbol
	from countries 
	inner join ProfileAddress on countries.id = ProfileAddress.id_country 
	inner join Profiles  on ProfileAddress.id_profile = Profiles.id 
	inner join ProfileTypes on Profiles.id_profileType = ProfileTypes.id 
	WHERE ProfileTypes.id  = 2;;`

	q_ListarPaisesCliente := []models.ListarPaisesCliente{}
	db.Select(&q_ListarPaisesCliente, query)
	...
	json.NewEncoder(w).Encode(q_ListarPaisesCliente)
	db.Close()
}

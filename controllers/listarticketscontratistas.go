...
func GetListarTicketsContratistas(w http.ResponseWriter, r *http.Request) {

	db, err := sqlx.Connect("mysql", configuraciones.ConnectionString)
	query := `select id, date_due, status, id_profile, id_contractor, id_servicessupplier, notes, amount_charged 
	from ServicesContracts
	where status = 'Activo';`

	q_ListarTicketsContratistas := []models.ListarTicketsContratistas{}
	db.Select(&q_ListarTicketsContratistas, query)
	...
	json.NewEncoder(w).Encode(q_ListarTicketsContratistas)
	db.Close()
}

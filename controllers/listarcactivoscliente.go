...
func GetListarActivosCliente(w http.ResponseWriter, r *http.Request) {
	...
	var ListarActivosCliente models.ListarActivosCliente
	var Resultado []models.ListarActivosCliente

	err = json.Unmarshal(body, &ListarActivosCliente)
	if err != nil {
		panic("fuck")
	}
	...
	query := `CALL listarTicketsActivosCliente(?);`
	rows, err := db.Queryx(query, ListarActivosCliente.Id)
	if err != nil {
		panic(err)
	}
	...
	for rows.Next() {
		...
		err = rows.StructScan(&ListarActivosCliente)
		Resultado = append(Resultado, ListarActivosCliente)

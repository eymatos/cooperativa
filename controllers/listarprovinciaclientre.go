...
func GetListarProvinciaCliente(w http.ResponseWriter, r *http.Request) {

	db, err := sqlx.Connect("mysql", configuraciones.ConnectionString)
	query := `select states.id, states.name, states.country_id, states.country_code, states.fips_code, states.iso2,
	states.latitude, states.longitude, states.created_at, states.updated_at, states.flag, states.wikidataid
	from states 
	inner join ProfileAddress on states.id = ProfileAddress.id_state 
	inner join Profiles on ProfileAddress.id_profile = Profiles.id 
	WHERE Profiles.id = 2;`

	q_ListarProvinciaCliente := []models.ListarProvinciaCliente{}
	db.Select(&q_ListarProvinciaCliente, query)
	...
	json.NewEncoder(w).Encode(q_ListarProvinciaCliente)
	db.Close()
}

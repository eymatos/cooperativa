...
func GetListarHistoricoCompletadoCliente(w http.ResponseWriter, r *http.Request) {

	fmt.Fprintf(w, "hola desde Crea Historico Completado Cliente")
	...
	var ListarHistoricoCompletadoCliente models.ListarHistoricoCompletadoCliente
	var Resultado []models.ListarHistoricoCompletadoCliente

	err = json.Unmarshal(body, &ListarHistoricoCompletadoCliente)
	if err != nil {
		panic("fuck")
	}
	...
	query := `CALL listarTicketsCompletadosCliente(?);`
	rows, err := db.Queryx(query, ListarHistoricoCompletadoCliente.Id)
	if err != nil {
		panic(err)
	}

	fmt.Fprintf(w, "Debug query")
	for rows.Next() {

		err = rows.StructScan(&ListarHistoricoCompletadoCliente)
		Resultado = append(Resultado, ListarHistoricoCompletadoCliente)
	...
	//result, err := db.NamedExec(query, ListarHistoricoCompletadoCliente)
	//ListarHistoricoCompletadoCliente = result

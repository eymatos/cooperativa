        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    <x-nav-link :href="Auth::user()->tipo == 2 ? route('admin.dashboard') : route('socio.dashboard')" :active="request()->routeIs('*.dashboard')">
                        {{ __('Dashboard') }}
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <div class="ml-3 relative flex items-center gap-4">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="text-xs font-bold text-red-500 hover:text-red-700 uppercase tracking-tighter border border-red-200 px-2 py-1 rounded hover:bg-red-50 transition">
                            {{ __('Salir') }}
                        </button>
                    </form>
                </div>

...
$cedula=$row_sexo['cedula'];
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "actualizar_e")) {
	$email=$_POST["correo"];
  $updateSQL = "UPDATE usuarios SET email='$email' WHERE cedula='$cedula'";
  mysql_select_db($database_sgstec, $sgstec);
...
<table width="100%" border="0" cellspacing="5" cellpadding="5" id="perfil1">
  <tr>
    <td colspan="2" id="encabezado">Cambio de correo electrónico</td>
    </tr>
  <tr>
    <td width="27%" style="font-weight: bold; padding-top: 20px;">Nueva correo electrónico:</td>
    <td width="73%" style="padding-top: 20px;"><input name="correo" type="text" />
      <input name="id" type="hidden" id="id" value="<?php echo $row_sexo['id']; ?>" /></td>
  </tr>
  </table>
          <div style="text-align: right; padding-right: 320px;"><input class="enviar" type="submit" value="Cambiar" /></div>
          <input type="hidden" name="MM_update" value="actualizar_e" />
          </form>
